...
class WebSocketClient(Thread):
    ...
    def connect(self):
        with self.lock:
            try:
                self.ws = connect(self.url, close_timeout=1)
    ...
    def send_to_server(self):
        ...
        while True:
            ...
            try:
                ...
                if data:
                    WebSocketClient.send_msg_queue.put(data)
    ...
    def run(self):
        t = Thread(target=self.send_to_server, daemon=True)
        t2 = Thread(target=self.check_connection, daemon=True)
        t.start()
        t2.start()
